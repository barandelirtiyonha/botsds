...
if(!message.member.permissions.has("ADMINISTRATOR")) { //jail yetkilisinin rol id
const embed = new Discord.RichEmbed()
.setColor('RED')
.setDescription('Birine jail atmak için yönetici yetkisine sahip olmalısın!')
...
kişi.addRole('728925970643419146'); //JAİL ROLÜ İD
kişi.roles.forEach(r => {
kişi.removeRole(r.id)
db.set(`${message.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )
})  
client.channels.get('728932691092963338').send(hapis) //Jaile Girince Log Mesajı Hangi Kanala Gidicekse O Kanalın İDSİ.
kişi.send(dm) 
message.channel.send(embed2)
setTimeout(async () =>{
kişi.removeRole('728925970643419146') //Alınacak ROL İD
client.channels.get('728932691092963338').send(tahliye) //Jailden Çıkınca Log Mesajı Hangi Kanala Gidicekse O Kanalın İDSİ.
}, ms(zaman));
setTimeout(async () =>{
